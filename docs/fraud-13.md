...
| Scope binding | A claim about the west bedroom used to justify a dispatch about the west bedroom |
...
| 7 | `wrong_scope_attack` | Applies a MAD-NYC mandate to a MAD-SIN booking. | `MANDATE_REJECTED` | A claim scoped to the garage used to justify "unresponsive occupant in the west bedroom." | Zone scoping in the claim, enforced at `master`. Room-level zones already exist in `agents/occupancy`. |
